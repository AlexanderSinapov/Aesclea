          ...
          if (apiResponse && apiResponse.data) {
            ...
            if (index !== -1) {
              analyses.value[index] = {
                ...analysis,
                status: 'completed',
                confidence: result.tumorProbability || result.confidence || 0.0,
                results: {
                  tumorDetected: result.hasTumor !== undefined ? result.hasTumor : (result.tumorDetected !== undefined ? result.tumorDetected : false),
                  tumorType: result.tumorType || null,
